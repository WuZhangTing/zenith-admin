...
export default function PaymentLedgerPage() {
  ...
  const accountColumns: ColumnProps<PaymentLedgerAccount>[] = [
    ...
    { title: '商户配置', dataIndex: 'channelConfigId', width: 160, render: (value: number) => merchantNameById.get(value) ?? `配置 #${value}` },
  ...
  const journalColumns: ColumnProps<PaymentJournal>[] = [
    ...
    { title: '应用', dataIndex: 'appId', width: 140, render: (value: number) => appNameById.get(value) ?? `应用 #${value}` },
    { title: '商户配置', dataIndex: 'channelConfigId', width: 150, render: (value: number) => merchantNameById.get(value) ?? `配置 #${value}` },
    { title: '币种', dataIndex: 'currency', width: 80 },
    {
