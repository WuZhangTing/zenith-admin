...
export const PaymentDisputeDTO = z
  .object({
    ...
    route: z.string().nullable().openapi({ description: '智能分流路由（urgent/manual/auto_refund_suggest；null=未分流）' }),
    priority: z.number().int().nullable().openapi({ description: '分流优先级，数值越大越紧急' }),
    slaHours: z.number().int().nullable().openapi({ description: '分流建议 SLA（小时）' }),
