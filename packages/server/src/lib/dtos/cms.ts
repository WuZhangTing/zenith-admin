...
export const CmsSiteImportResultDTO = z
  .object({
    ...
    counts: z.object({
      ...
      interactionQuestions: z.number().int(),
      resourceFolders: z.number().int(),
      resources: z.number().int(),
      models: z.number().int(),
      modelFields: z.number().int(),
      friendLinkGroups: z.number().int(),
...
export const CmsOpenContentDTO = z
  .object({
    ...
    expireAt: z.string().nullable().optional().openapi({ description: '内容公开到期时间；客户端应在到期时停止展示' }),
