...
export const AiMessageDTO = z
  .object({
    ...
    toolCalls: z
      .array(z.object({ name: z.string(), arguments: z.string(), result: z.string() }))
      .nullable()
      .openapi({ description: '工具调用过程（名称/参数/结果）' }),
    references: z
      .array(z.object({ docName: z.string(), content: z.string(), score: z.number() }))
      .nullable()
      .openapi({ description: '知识库检索引用' }),
...
export const UserAiConfigDTO = z
  .object({
    ...
    headers: z.record(z.string(), z.string()).nullable().openapi({ description: '自定义请求头' }),
    models: z.array(z.string()).openapi({ description: '启用的模型列表' }),
    defaultModel: z.string().nullable().openapi({ description: '默认模型' }),
    modelSettings: ModelSettingsDTO,
    providerOptions: z.record(z.string(), z.record(z.string(), z.unknown())).nullable().openapi({ description: '服务商特定选项（按 provider 分组透传）' }),
    capabilities: z.object({
      vision: z.boolean().optional(),
      tools: z.boolean().optional(),
      contextWindow: z.number().optional(),
    }).nullable().openapi({ description: '模型能力标签' }),
