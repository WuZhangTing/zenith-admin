 ...
 * 连接即在线；上线补推全部 pending 指令；断开即离线（HTTP 心跳设备可重建在线态）。
 *
 * 帧协议（JSON）：
...
import { z } from 'zod';
import {
  IOT_WS_FRAME_TYPES, iotTelemetryIngestSchema, iotCommandAckSchema,
    ...
    upgradeWebSocket(async (c) => {
      ...
      return {
        onOpen(_evt, ws) {
          ...
          // 在线登记 + 上线补推 pending 指令（推完统一标 delivered）
          void (async () => {
            await touchDevice(d);
            const pendings = await getPendingCommandPayloads(d.id);
