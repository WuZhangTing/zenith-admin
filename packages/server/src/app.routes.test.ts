...
describe('路由表快照', () => {
  it('完整路由表保持不变', async () => {
    ...
    // 断言本身是纯内存排序，耗时几乎全在 await import('./app') 转译整套模块图。
    // 独占机器约 74s，而发布流程会让 lint / build / docs 三路构建与本套件同时抢 CPU
    // （同为转译密集），实测该情形下需 90s+。随路由与页面持续增长，300s 已被同类的
    // app.contract.test.ts 撞破过一次，故与其一并抬到 480s 保持余量口径一致。
    // 真正卡死（如顶层 await 死锁）仍会在 8 分钟内失败。
  }, 480_000);
});
