...
import type { CmsLinkEntityType, CmsLinkRef } from '@zenith/shared';
import { db } from '../../db';
...
type ChannelTarget = { id: number; path: string; type: string; linkUrl: string | null };
...
function entityIdsOf(refs: Iterable<CmsLinkRef>, entityType: CmsLinkEntityType): Set<number> {
  ...
  for (const ref of refs) {
    if (ref.kind === 'entity' && ref.entityType === entityType) ids.add(ref.id);
  }
  return ids;
}
...
async function loadContentTargets(siteId: number, seedIds: Set<number>): Promise<Map<number, ContentTarget>> {
  ...
  for (let hop = 0; hop < MAX_HOPS && pending.size > 0; hop++) {
    ...
    for (const row of rows) {
      ...
      if (ref?.kind === 'entity' && ref.entityType === 'content' && !loaded.has(ref.id)) next.add(ref.id);
    }
    pending = next;
  }
  return loaded;
...
export async function buildCmsLinkResolver(
  ...
  if (seedContentIds.size > 0 || seedChannelIds.size > 0) {
 ...
 */
export async function describeCmsLink(siteId: number, raw: string | null | undefined): Promise<{
  ...
  const value = raw?.trim() ?? '';
  const ref = parseCmsLink(value);
  if (!ref) return { kind: 'invalid', label: value, targetId: null, exists: false };
  if (ref.kind === 'external') return { kind: 'external', label: ref.url, targetId: null, exists: true };
  if (ref.kind === 'internal') return { kind: 'internal', label: ref.path, targetId: null, exists: true };
