...
- **任务中心**：统一异步任务框架（批量导入/导出、报表生成、静态化等长耗时操作），带进度推送、断点续跑、自动重试、取消与行级明细
...
- **决策表 / 决策流**：可视化规则编排与执行，支持决策执行记录追溯
- **名单库**：黑白名单等名单集中管理，供风控与业务规则引用
...
- **仪表盘**：拖拽式仪表盘搭建、收藏、订阅推送与嵌入分享
- **打印报表 / 填报**：打印模板设计、数据填报模板与填报记录
...
- **应用与凭证**：开放应用管理、我的应用、API Scope 授权范围
- **调用治理**：限流套餐、调用统计、签名验签
...
- **内容生产**：内容管理与编辑、内容模型（自定义字段）、素材中心、标签 / 碎片 / 友情链接、采集中心、页面搭建（区块 ACL）
- **内容治理**：敏感词库、易错词库、评论审核、SEO 管理、检索管理（全文搜索）
- **前台渲染**：内置 React TSX 主题（`default` 企业门户 / `docs` 文档站）服务端 SSR 渲染，支持动态 / 混合 / 全静态三种静态化模式
