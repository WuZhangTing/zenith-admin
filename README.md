...
- **消息频道**：系统号 / 运营号订阅体系，群发消息（草稿 / 定时 / 撤回）、频道菜单与自动回复、客服工作台（会话聚合 / 快捷回复 / 满意度 / 绩效统计）、频道数据看板
- **个人消息**：「我的消息」收件箱与「公告中心」个人页，导航未读徽标全布局展示
- **实时推送**：基于 WebSocket 的新通知实时推送，前端自动重连（指数退避）
- **消息中心（IM）**：内置即时通讯，单聊/群聊、语音消息、WebRTC 音视频通话、投票、消息转发（逐条/合并）、@提及、表情回应、卡片消息、未读计数、会话置顶与收藏、媒体库、消息搜索
...
- **审批中心**：发起工作台（按分类快速发起）、待我审批（通过 / 驳回 / 转办 / 委派 / 加签减签 / 退回 / 催办 / 手写签名）、我的申请、抄送我的、我已办、审批代理（委托代办）
...
- **流程自动化 / 定时发起**：基于触发条件的自动化规则与 Cron 定时发起流程实例
- **事件订阅 / 触发器 / 连接器**：Webhook 订阅端点管理、触发器执行记录、外部系统连接器统一外呼，支持外部审批回调与补偿 / Saga 编排
- **流程监控**：实例监控与跨实例任务监控双视角、流程轨迹、统计分析、逾期任务与导出
...
- **知识库（RAG）**：知识库管理、文档自动分块与向量化检索、URL 抓取网页正文入库，对话可挂载知识库引用回答
- **智能体 / AI 工具**：自定义智能体配置与对话；HTTP 工具（函数调用）供对话与智能体调用外部接口
- **模型评测**：评测集管理、异步评测运行与逐条结果查看、多模型对比投票（竞技场）
- **AI 服务商**：多模型服务商配置与测试连接，支持用户级个性化配置
- **提示词模板**：可复用提示词模板管理，对话中一键套用
- **用量统计**：按模型 / 用户统计 Token 用量，支持日期范围查询
- **对话审计 / AI 反馈**：管理端对话内容审计查询、用户反馈收集与查看
...
- **内容生产**：内容管理与编辑、内容模型（自定义字段）、素材中心、标签 / 碎片 / 友情链接、采集中心、页面搭建与可复用页面部件（区块 ACL）
- **内容治理**：敏感词库、易错词库、评论审核、SEO 管理、检索管理（全文搜索）
- **前台渲染**：内置 React TSX 主题（`default` 企业门户 / `docs` 文档站 / `gov-portal` 政府门户 / `magazine` 资讯杂志）服务端 SSR 渲染，Theme API 支持主题参数声明取数，支持动态 / 混合 / 全静态三种静态化模式
...
- **主题外观**：亮色 / 暗色 / 跟随系统，深色底色分区（侧边栏 / 顶部 / 主区域）独立档位，19 种预设主题色 + 自定义颜色，灰色 / 色弱模式，页面水印
