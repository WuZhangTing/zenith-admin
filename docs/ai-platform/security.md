...
| 规则 | 默认 | 说明 |
| --- | --- | --- |
| `ai_chat_send` | 15 次 / 分钟 | 聊天发送接口按用户限流（Arena 对比同样适用），可在「限流规则」页调整 |
| `ai_share_view` | 60 次 / 分钟 | 分享只读页按 IP 限流，可在「限流规则」页调整 |
| `ai_daily_token_quota` | 0（不限） | 每用户每日 token 配额（系统配置），超限返回 429 |

## 对话分享安全

- 分享 token 为 192-bit 随机值，免登录访问 `/public/ai-chat/{token}`（后端接口 `GET /api/ai/public/chat/{token}`）。
- 支持有效期 0–365 天（0 = 永久），可随时撤销；同一会话仅保留一个有效分享，重新生成会替换旧链接；过期访问返回 410。
- 分享页只读（消息按时间序展示），不暴露用户信息、模型配置与反馈数据。
