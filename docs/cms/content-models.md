...
管理入口：`/cms/models`（权限 `cms:model:*`）。表：`cms_models` / `cms_model_fields`。普通操作员的列表、详情、字段、引用、更新和删除请求都带 `siteId` scope；平台管理员可以省略 scope 查看平台共享模型的全局引用。
...
字段标识 `name` 在同一模型内必须唯一；模型字段替换在一个事务中完成，重复标识直接返回 400。手工选项的 value 由发布校验检查，字典来源只展开启用的字典项。
...
- 修改模型字段后，存量内容的 `extend` 不迁移——新字段读为空、被删字段成为不再解释的冗余键；服务端按模型归属为受影响站点提交重建任务（平台共享模型覆盖平台站点范围），静态页异步按新定义更新
