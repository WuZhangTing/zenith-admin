...
记录批准后自动提交任务中心异步任务 **`report-fill-sync`**，把该模板**全部批准记录**同步为一个静态**生成数据集**（整体替换，非追加），并在模板与记录上回填 `generatedDatasetId`、`syncTaskId`、`syncStatus`。生成数据集与普通数据集一样可用于仪表盘、打印报表与预警。

> 金额类字段按表单定义处理，业务金额建议使用**整数分**。

## 对账兜底（自动自愈）

工作流回调、任务提交等环节偶发失败时，系统每 5 分钟运行对账任务 `report-fill-workflow-reconcile` 自动修复三类悬挂状态：

1. **补链流程终态**：流程实例已审结但记录状态未更新的，按流程结果补置 approved/rejected；
2. **恢复缺失关联**：已提交但丢失流程实例关联的记录，重新发起或重新挂接流程；
3. **重提同步任务**：已批准但数据集同步任务缺失/失败的，重新提交 `report-fill-sync`。

每轮有界执行（单类最多 100 条），幂等可重复运行，无需人工干预。
