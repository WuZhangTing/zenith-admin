...
| [类 Excel 打印报表](./print-reports) | Univer 单元格设计器、单元格表达式、纵向扩展、多数据集/交叉表/子报表、套打与 Excel / PDF / Word 导出 |
...
| 外部 API / 数据库连不上内网地址 | 出站默认拒绝私网、链路本地、云元数据与保留地址，需运维配置 `REPORT_OUTBOUND_PRIVATE_ALLOWLIST` 白名单（见[数据源接入](./datasources)） |
