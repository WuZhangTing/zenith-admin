...
- 会话回放开启时自动携带当前**回放 ID**（`replayId`），错误与录像精确关联。
...
报错时刻若回放录制处于活跃状态，错误事件会携带精确的回放 ID：

- Issue 详情「最近事件」与「错误事件」详情弹窗中，携带回放的事件显示**「查看会话回放」**按钮，跳转回放中心深链（`/analytics/replays?replay={id}`）直接播放报错前后的操作录像。
- 反向：回放详情的关联错误列表可跳回对应 Issue（`?issue={groupId}` 深链）。
- 事件 context 携带服务端 `requestId` 时另有「查看服务端链路」按钮跳转链路追踪，三者共同构成「录像 + 前端堆栈 + 服务端链路」的完整排障现场。

回放的录制模式、触发方式与隐私策略见 [会话回放](./session-replay)。
