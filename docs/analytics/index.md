...
Zenith Admin 内置**前端数据分析与错误监控系统**，无需外部服务即可完成行为采集、多维分析、埋点治理、A/B 实验、用户分群触达与错误监控告警。本章按模块拆分讲解功能与用法。

## 章节导航

| 文档 | 内容 |
|------|------|
| [埋点采集 SDK](./tracking) | 自动采集（页面/点击/性能/API/Rage Click/滚动深度/白屏）、手动埋点 API、声明式 `data-track`、上报字段、远程配置热更新、批量上报、离线重试、隐私脱敏、A/B 实验分流、站点 siteKey、服务端权威事件与 403/404 语义事件 |
...
| [数据管理](./data-management) | 事件明细与导出、事件字典（Tracking Plan）、租户级事件覆盖、埋点质量看板、事件调试、用户分群（CRUD + 异步物化）、站点管理与配额、每日聚合、采集设置、错误忽略规则、数据保留策略、报表中心复用 |
| [错误监控](./error-monitoring) | Issue 分组模型、捕获范围、Source Map 堆栈还原、行为面包屑、状态流转/指派、告警规则与触发历史 |
| [架构与数据模型](./architecture) | 21 张数据表、路由与 Service 分层、数据链路、WebSocket 事件、定时任务与异步任务、权限码、多租户隔离 |
| [容量演进与架构触发条件](./capacity) | PostgreSQL 基线、触发阈值、自查 SQL、PG 分区优先路径、队列 + OLAP 后续路径、不立项能力评估 |
...
- **多租户隔离**：行为与错误数据按 `tenantId` 隔离；匿名上报按站点 `siteKey` 归属租户，错误指纹含租户与环境因子保证分组全局唯一，事件字典按事件名全局治理。

## 数据流

```text
前端 Tracker SDK (@zenith/analytics-sdk)
  · 自动采集 + Web Vitals + API 监控 + 错误上报 + 批量上报 + 离线重试 + 远程配置热更新
...
PostgreSQL（21 张分析与监控表）+ Redis（站点配额计数）
        ↓ 聚合分析接口
GET  /api/analytics/*                概览/趋势/会话/漏斗/留存/路径/实时/事件分析/实验报告…
GET  /api/frontend-errors/*          概览/分组/详情(还原)/事件/告警规则/告警历史…
        ↓ 定时任务（pg-boss）+ 任务中心异步任务
analyticsRollupDaily / analyticsRetention / analyticsSegmentRefresh / evaluateErrorAlerts
聚合重建 / 分群物化 / 触达执行
```
