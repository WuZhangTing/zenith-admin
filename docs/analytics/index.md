...
| [行为分析](./behavior) | 概览 KPI、趋势（支持环比对比）、实时、页面停留、功能使用、会话与会话时间轴、漏斗（有序转化 + 转化窗口 + 分群限定 + 保存报表）、留存（真实首访/窗口首现双口径 + 日/周/月粒度）、路径、用户时间线、维度分布与双维交叉、Web Vitals、点击分布、事件分析工作台（含属性过滤）、A/B 实验（含显著性检验与 SRM）、分群触达 |
