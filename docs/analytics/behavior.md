...
`GET /api/analytics/path?days=N&maxSteps=N&limit=N&startPage=/xxx` —— 页面跳转路径，前端以**桑基图**展示，下方配跳转明细表。

节点身份是**「步序 × 页面」**而非单纯页面：`s1:/` 与 `s3:/` 是两个节点。这是关键——按页面建模时
互相跳转（`/ ⇄ /profile`）会构成环，而桑基布局无法表达回边；按步序拆开后链路只会从第 N 步流向第 N+1 步，天然无环。

| 参数 | 说明 |
|------|------|
| `days` | 分析窗口天数，1–365，默认 30 |
| `maxSteps` | 路径深度（保留到第几步），2–10，默认 5 |
| `limit` | **每一步**保留的最高跳转链路数，1–30，默认 8 |
| `startPage` | 可选起点页；指定后以该页在会话中**首次出现**的位置作为第 1 步重新编号 |

返回 `{ nodes, links, maxStep }`，`nodes` 为 `{ id, label, value, step }`、`links` 为 `{ source, target, value, step }`。

口径要点：

- **连续重复页面折叠**：同一页面连续上报（刷新、局部跳转）算一步，不会产生自环。
- **退出节点**：会话在某步结束时链路指向 `$exit`（常量 `ANALYTICS_PATH_EXIT_PAGE`，前端渲染为「退出」）。
  丢掉这部分流量会让桑基图凭空变细，看不出每一步流失了多少。
- **`session_id` 为空的事件被排除**：无法还原会话内顺序，纳入会把不相关访问串成假路径。
- 前端把同一页面在不同步序上渲染成同一颜色，便于识别路径中反复出现的页面；退出节点为灰色。
  节点层级由 `step` 强制指定（`setNodeLayer`），否则布局会把没有出边的退出节点推到最右列，
  看起来像是最后一步才流失。
