...
| 监控告警 | 基于阈值的告警规则引擎，覆盖基础设施（CPU/内存/磁盘/Swap/负载/网络/磁盘 IO/QPS/HTTP 错误率/日志 ERROR·WARN 频率）、工作流健康度、支付与开放平台四类共 30 项指标；持续 N 分钟超阈触发、自动恢复解除、静默期防轰炸，支持邮件/Webhook/站内信/群机器人多通道；告警事件可按指标/级别/状态/处理状态筛选，并提供认领与处理闭环 |
...
| 日志文件 | 服务端日志文件列表、查看、下载、删除、正则/grep 搜索、实时追踪与暂停、复制导出、告警联动跳转 |
