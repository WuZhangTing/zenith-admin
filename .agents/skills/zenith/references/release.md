...
四路全部 `exit code 0` 方可继续。并行墙钟取决于最长的一路（通常是 build），实测 3-8 分钟（视机器负载而定），串行则需 7-9 分钟以上。任一路失败会立即终止其余任务，修复后可只重跑失败的那条命令。

### 读懂 `--kill-others-on-fail` 的输出（易误判）

任一路失败时 `concurrently` 会**主动杀掉**其余三路，被杀的那几路同样打印非 0 退出码。必须看结尾 Timings 表的 `killed` 列区分，**不要**看到多路飘红就以为崩了多处：
...
按上表只重跑真失败的那条命令，不必整套重来。

### 并发度：并行保留，但依赖两个已设好的旋钮

曾出现「四路并行时 server 测试超时」，容易误判为并行验证本身有害而想改回串行。**改串行并不能解决**——单独跑 `npm test`（零外层并发）同样超时。根因是转译成本，需要两个旋钮共同压住，二者缺一都会复发：

| 旋钮 | 位置 | 解决的是 | 实测 |
| --- | --- | --- | --- |
| `maxWorkers: 8` | `packages/server/vitest.config.ts` | vitest 默认 worker 数 = 核数−1，每个 worker 独立转译整套 app（267 个路由文件），核多时重复转译反超并行收益 | 单独跑 `npm test`：307s ❌ → 113s ✅ |
| `300_000` 超时 | `src/app.routes.test.ts` | 该用例耗时几乎全在 `await import('./app')`，恰与 lint / build / docs 三路争抢同一种（转译）资源 | 四路并行：120s 撞线 ❌ → 全绿 ✅ |

两者都已配置好，**本步骤无需额外传参**。日后若再遇测试超时：先确认是否为超时（而非断言失败）且单独跑能过，再按 [troubleshooting.md → 测试超时](./troubleshooting.md) 调这两个旋钮，而**不是**删掉这里的外层并行。
