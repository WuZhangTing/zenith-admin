...
- **搜索栏公共按钮**（Step 8）：查询 / 重置 / 新增 / 刷新按钮统一使用 `packages/web/src/components/toolbar-controls.tsx` 的 `SearchButton` / `ResetButton` / `CreateButton` / `RefreshButton`，**禁止**手写 `<Button type="primary" icon={<Search size={14} />} onClick={...}>查询</Button>` 这类字面量（曾在 165 个文件里复制 513 次）。文案不同时用 children 覆盖：`<CreateButton onClick={openCreate}>新增规则</CreateButton>`；文案与默认值相同则写自闭合。**例外**：仅仅复用同一图标的独立操作（如「测试发送」「生成链接」「发起分账」）以及视觉本就不同的写法（`theme="borderless"` / `size="small"` / 其他图标）保持原生 `Button`——将来改「新增」按钮图标不应连带改掉它们
...
- **mutation 精确失效**（Step 8）：域 hooks 的 `onSuccess` 按真实副作用失效，**禁止**无条件 `invalidateQueries({ queryKey: xxxKeys.all })` —— 它会把同根下的详情、统计、日志、下拉源一并打掉。写接口与详情接口同源时用 `setQueryData(detail(id), saved)` 回填；删除用 `removeQueries(detail(id))`（实体已不存在，失效会导致必然 404 的回源）；判据是「有没有已挂载的查询读了这次被改动的状态」，而非接口像不像命令。确需全域失效（批量覆盖、切租户、全量导入）须在注释写明理由。注意失效**未挂载**的缓存代价接近零（只标脏、不回源），所以该失效的别漏 —— 欠失效导致的陈旧 UI 比多失效更危险。详见 [crud-frontend.md 缓存一致性契约](./crud-frontend.md)
- **query key 结构**（Step 8）：`xxxKeys.all` 只能是**本域自己的根**（`['report','dashboards']` 而非 `['report']`）；独立生命周期的子资源另起命名空间（群成员用 `['chat','group-members',id]`，不要嵌在 `['chat','conversations',id,...]` 之下，否则「刷新会话列表」会因前缀匹配连带打掉成员名单）；同一实体的多变体查询导出前缀键（`detailOf(id)` / `dataOf(id)` / `lookupPrefix`）供精确覆盖；静态 lookup、数据库元数据、昂贵的派生取数必须与列表分处不同前缀
- **下拉源归属所有者域**（Step 8）：**禁止**用本域 key 去请求别域资源（如以 `announcementKeys` 为键拉 `/api/roles/all`）——所有者域增删改时没有任何来源会失效它，界面会静默显示旧列表。一律复用 `useAllRoles` / `useFlatDepartments` / `useAllUsers` / `useAllPositions` / `useDictItems` 等共享 lookup hook，需要特定选项结构时对其结果做 `useMemo` 派生
- **回填的可见性红线**（Step 8）：`setQueryData(detail(id), saved)` 仅限写接口与详情接口同源。详情做了按查看者脱敏（用户域：写接口 `mapUser` 明文 / 详情 `mapUserWithMask`，回填会泄露明文手机号邮箱）、详情多出关联数据（公告的收件人与附件）、写接口不回传编辑过的关联字段（角色的 `menuIds`，回填会清空菜单勾选）、列表/树含聚合字段（部门树、`userCount`）四种情形**必须**改为失效 `detail(id)`
- **失效行为需可证伪**（Step 8）：域 hooks 的测试用 `packages/web/src/test-utils/query-harness.ts`（`ApiRecorder` 记录并桩化请求、`observeFetches` 区分「真的重拉」与「只是被标脏」、`isFresh` / `isInvalidated` / `hasCacheEntry` 断言缓存），断言必须落在实际请求数、真正进入 fetching 的查询、缓存内容与新鲜度上；**禁止**只 spy「调用了 `invalidateQueries(某 key)`」—— `all` 是 `detail` 的前缀，这类断言在冗余的广播写法和被改坏的收敛写法下都会通过
- **失效粒度防回潮**（Step 8）：web 包 `npm run lint` 包含 `scripts/check-invalidation-baseline.mjs`，按 `scripts/invalidation-baseline.json` 对域 hooks 中 mutation `onSuccess` 内的 `xxxKeys.all` 做**只减不增**校验（页面 `handleSearch` / `handleReset` 与 keys 定义不在扫描范围）。确需广播须先在注释写明理由，再 `node scripts/check-invalidation-baseline.mjs --update`；收敛完一个域也要 `--update` 锁住额度
